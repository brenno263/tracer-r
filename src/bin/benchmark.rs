...
fn main() {
	...
	let num_spheres = usize::from_str_radix(&args[1], 10).or(Err(&args[1])).expect("invalid number of spheres");
...
fn setup(num_spheres: usize) -> (Raytracer, Camera, Vec<Primitive>, (usize, usize)) {
	...
	let elements = random_spheres(num_spheres, Bounds { min_point: V3::new(-10., -10., 10.), max_point: V3::new(10., 10., 24.) });
	...
	where F: Fn()->()
{
	...
	println!("Ran {} in {:.3} seconds", description, elapsed.as_secs_f32() / 5.);
}
